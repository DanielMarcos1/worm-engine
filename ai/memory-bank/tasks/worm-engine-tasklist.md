# Worm Engine Task List (v1.0.0 SOTA Integration)

## Technical Notes
- **wgpu alignment**: Be aware of `wgpu` 16-byte alignment issues when passing data to compute shaders (use flat `array<f32>` instead of `vec3<f32>` for WGSL shaders to match standard Rust `bytemuck` arrays).
- **SIMD Crate**: Use the `wide` crate for SIMD vectorization (specifically `wide::f32x4`) instead of `std::simd` to ensure cross-platform compatibility without nightly requirements. Explicitly use `.to_array()` instead of `.into()` to prevent type inference compilation errors.
- **Rayon Parallelism**: When using `rayon` for task-based parallelism, appropriately chain iterators using `.par_iter_mut().zip(...)` for multiple mutable Struct of Arrays (SoA) vectors.
