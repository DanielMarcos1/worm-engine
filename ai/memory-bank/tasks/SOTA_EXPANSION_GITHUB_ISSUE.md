...
## Labels
enhancement, architecture, physics, performance
...
- Feature branches correctly pass all test coverage and cross-platform verification checks.

### Technical Constraints
- SIMD vectorization must use the `wide` crate over SoA chunks (not `std::simd`).
- Iterating multiple mutable SoA arrays in rayon must chain `.par_iter_mut().zip(...)`.
- WGSL shaders must avoid 16-byte alignment crashes by using flat `array<f32>`.
