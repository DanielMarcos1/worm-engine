...
pub fn update(components: &mut RigidBodyComponents, index: usize, dt: f32) {
    ...
    let accel_x = components.forces_x[index] / mass;
    let accel_y = components.forces_y[index] / mass;
    let accel_z = components.forces_z[index] / mass;

    components.accelerations_x[index] = accel_x;
    components.accelerations_y[index] = accel_y;
    components.accelerations_z[index] = accel_z;

    components.velocities_x[index] += accel_x * dt;
    components.velocities_y[index] += accel_y * dt;
    components.velocities_z[index] += accel_z * dt;
