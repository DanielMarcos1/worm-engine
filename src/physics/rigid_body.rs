...
pub fn apply_gravity(components: &mut RigidBodyComponents, index: usize) {
    ...
    let force_x = GRAVITY.x * mass;
    let force_y = GRAVITY.y * mass;
    let force_z = GRAVITY.z * mass;

    components.forces_x[index] += force_x;
    components.forces_y[index] += force_y;
    components.forces_z[index] += force_z;
}
