...
pub fn update(components: &mut RigidBodyComponents, index: usize, dt: f32) {
    ...
    let mut vel = Vector3d::new(components.velocities_x[index], components.velocities_y[index], components.velocities_z[index]);
    vel = vel.add(&accel.scale(dt));
    components.velocities_x[index] = vel.x;
    components.velocities_y[index] = vel.y;
    components.velocities_z[index] = vel.z;
