...
impl World {
    ...
    pub fn add_body(&mut self, shape: Polygon, mass: f32) {
        ...
        self.positions.push(Vector3d::zero());
    ...
    pub fn step(&mut self) {
        ...
        let chunk_size = 4;
        let num_bodies = self.masses.len();
        let remainder = num_bodies % chunk_size;
        let exact_len = num_bodies - remainder;

        // Process in chunks of 4 for SIMD vectorization
        self.shapes[..exact_len].par_chunks_mut(chunk_size)
            .zip(self.masses[..exact_len].par_chunks_mut(chunk_size))
            .zip(self.velocities[..exact_len].par_chunks_mut(chunk_size))
            .zip(self.accelerations[..exact_len].par_chunks_mut(chunk_size))
            .zip(self.forces[..exact_len].par_chunks_mut(chunk_size))
        ...
        if remainder > 0 {
            ...
            for i in start..end {
                ...
                let force = self.forces[i].add(&gravity_force);
                ...
                for vertex in &mut self.shapes[i].vertices {
                    *vertex = vertex.add(&velocity.scale(dt));
                }

                self.forces[i] = Vector3d::zero();
            }
        }
        ...
        for i in 0..num_bodies {
            for j in (i + 1)..num_bodies {
                ...
                if self.shapes[i].vertices.is_empty() || self.shapes[j].vertices.is_empty() { continue; }

                let p1 = self.shapes[i].vertices[0];
                let v1 = self.velocities[i];
                let r1 = 1.0; // Approximation

                let p2 = self.shapes[j].vertices[0];
                ...
                if let Some(toi) = calculate_toi_sphere_sphere(p1, v1, r1, p2, v2, r2, dt) {
                    ...
                    let v1_proj = self.velocities[i].dot(&collision_normal);
                    let v2_proj = self.velocities[j].dot(&collision_normal);

                    if v1_proj < 0.0 {
                        self.velocities[i] = self.velocities[i].subtract(&collision_normal.scale(v1_proj));
                    }
                    if v2_proj > 0.0 {
                        self.velocities[j] = self.velocities[j].subtract(&collision_normal.scale(v2_proj));
                    }
                }
            }
        }
...
mod tests {
    ...
    fn test_add_body() {
        ...
        world.add_body(poly, 10.0);

        assert_eq!(world.masses.len(), 1);
        assert_eq!(world.masses[0], 10.0);
        assert_eq!(world.shapes.len(), 1);
        assert_eq!(world.positions.len(), 1);
        assert_eq!(world.velocities.len(), 1);
        assert_eq!(world.accelerations.len(), 1);
        assert_eq!(world.forces.len(), 1);
        assert_eq!(world.active_entities.len(), 1);
        assert!(world.active_entities[0]);
    }

    #[test]
    fn test_step() {
        let mut world = World::new(0.016); // ~60fps

        // Add 5 bodies to test SIMD (chunk of 4) + remainder (1)
        for i in 0..5 {
            let poly = Polygon::new(vec![Vector3d::new(0.0, 0.0, 0.0)]);
            world.add_body(poly, 1.0);

            // Add a force to see it move
            world.forces[i] = Vector3d::new(0.0, 10.0, 0.0);
        }
        ...
        // Check first 4 (SIMD)
        for i in 0..4 {
            // Velocity should be updated due to force + gravity
            assert!(world.velocities[i].magnitude() > 0.0);
            // Forces should be zeroed
            assert_eq!(world.forces[i].magnitude(), 0.0);
        }

        // Check remainder (1)
        assert!(world.velocities[4].magnitude() > 0.0);
        assert_eq!(world.forces[4].magnitude(), 0.0);
    }
}
