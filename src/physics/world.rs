    ...
    pub fn step(&mut self) {
        ...
        self.bodies.shapes[..exact_len].par_chunks_mut(chunk_size)
            ...
            .for_each(|((((((((((shapes, masses), v_x), v_y), v_z), a_x), a_y), a_z), f_x), f_y), f_z)| {
                ...
                // Load forces
                let mut fx_simd = f32x4::from([f_x[0], f_x[1], f_x[2], f_x[3]]);
                let mut fy_simd = f32x4::from([f_y[0], f_y[1], f_y[2], f_y[3]]);
                let mut fz_simd = f32x4::from([f_z[0], f_z[1], f_z[2], f_z[3]]);

                // Apply gravity (F = F + mg)
                fx_simd = fx_simd + (grav_x * mass_simd);
                fy_simd = fy_simd + (grav_y * mass_simd);
                fz_simd = fz_simd + (grav_z * mass_simd);

                // Calculate acceleration (a = F / m)
                let ax_simd = fx_simd * inv_mass;
                let ay_simd = fy_simd * inv_mass;
                let az_simd = fz_simd * inv_mass;

                // Load velocities
                let mut vx_simd = f32x4::from([v_x[0], v_x[1], v_x[2], v_x[3]]);
                let mut vy_simd = f32x4::from([v_y[0], v_y[1], v_y[2], v_y[3]]);
                let mut vz_simd = f32x4::from([v_z[0], v_z[1], v_z[2], v_z[3]]);

                // Update velocities (v = v + a * dt)
                vx_simd = vx_simd + (ax_simd * dt_simd);
                vy_simd = vy_simd + (ay_simd * dt_simd);
                vz_simd = vz_simd + (az_simd * dt_simd);

                // Store back
                let a_x_arr: [f32; 4] = ax_simd.into();
                let a_y_arr: [f32; 4] = ay_simd.into();
                let a_z_arr: [f32; 4] = az_simd.into();

                let v_x_arr: [f32; 4] = vx_simd.into();
                let v_y_arr: [f32; 4] = vy_simd.into();
                let v_z_arr: [f32; 4] = vz_simd.into();
