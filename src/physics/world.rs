    ...
    pub fn step(&mut self) {
        ...
        if remainder > 0 {
            ...
            for i in start..end {
                ...
                let mut f_x = self.bodies.forces_x[i];
                let mut f_y = self.bodies.forces_y[i];
                let mut f_z = self.bodies.forces_z[i];

                f_x += crate::physics::constants::GRAVITY.x * mass;
                f_y += crate::physics::constants::GRAVITY.y * mass;
                f_z += crate::physics::constants::GRAVITY.z * mass;
