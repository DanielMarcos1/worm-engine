...
impl World {
    ...
    pub fn step(&mut self) {
        ...
        self.bodies.shapes[..exact_len].par_chunks_mut(chunk_size)
            ...
            .for_each(|((((((((((shapes, masses), v_x), v_y), v_z), a_x_arr), a_y_arr), a_z_arr), f_x), f_y), f_z)| {
                ...
                // Store back
                let a_x_res: [f32; 4] = a_x_simd.into();
                let a_y_res: [f32; 4] = a_y_simd.into();
                let a_z_res: [f32; 4] = a_z_simd.into();

                let v_x_res: [f32; 4] = v_x_simd.into();
                let v_y_res: [f32; 4] = v_y_simd.into();
                let v_z_res: [f32; 4] = v_z_simd.into();
                ...
                for i in 0..4 {
                    ...
                    for vertex in &mut shapes[i].vertices {
                        vertex.x += v_x_res[i] * dt;
                        vertex.y += v_y_res[i] * dt;
                        vertex.z += v_z_res[i] * dt;
                    }
                }
            });
        ...
        if remainder > 0 {
            ...
            for i in start..end {
                ...
                self.bodies.velocities_x[i] += a_x * dt;
                self.bodies.velocities_y[i] += a_y * dt;
                self.bodies.velocities_z[i] += a_z * dt;

                let v_x = self.bodies.velocities_x[i];
                let v_y = self.bodies.velocities_y[i];
                let v_z = self.bodies.velocities_z[i];
