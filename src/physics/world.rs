...
impl World {
    ...
    pub fn step(&mut self) {
        ...
        self.bodies.shapes[..exact_len].par_chunks_mut(chunk_size)
            ...
            .for_each(|((((((((((shapes, masses), vel_x), vel_y), vel_z), acc_x), acc_y), acc_z), frc_x), frc_y), frc_z)| {

                let mass_simd = f32x4::from([masses[0], masses[1], masses[2], masses[3]]);
                let inv_mass = f32x4::splat(1.0) / mass_simd;

                // Load forces
                let mut f_x = f32x4::from([frc_x[0], frc_x[1], frc_x[2], frc_x[3]]);
                let mut f_y = f32x4::from([frc_y[0], frc_y[1], frc_y[2], frc_y[3]]);
                let mut f_z = f32x4::from([frc_z[0], frc_z[1], frc_z[2], frc_z[3]]);
                ...
                for i in 0..4 {
                    ...
                    // Update vertices
                    let vel_vec = Vector3d::new(v_x_arr[i], v_y_arr[i], v_z_arr[i]);
