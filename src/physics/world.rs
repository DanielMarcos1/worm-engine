...
impl World {
    ...
    pub fn step(&mut self) {
        ...
        self.bodies.shapes[..exact_len].par_chunks_mut(chunk_size)
            ...
            .for_each(|((((((((((shapes, masses), vels_x), vels_y), vels_z), accels_x), accels_y), accels_z), forces_x), forces_y), forces_z)| {
                ...
                // Load velocities
                let mut v_x = f32x4::from([vels_x[0], vels_x[1], vels_x[2], vels_x[3]]);
                let mut v_y = f32x4::from([vels_y[0], vels_y[1], vels_y[2], vels_y[3]]);
                let mut v_z = f32x4::from([vels_z[0], vels_z[1], vels_z[2], vels_z[3]]);
                ...
                for i in 0..4 {
                    accels_x[i] = a_x_arr[i];
                    accels_y[i] = a_y_arr[i];
                    accels_z[i] = a_z_arr[i];
