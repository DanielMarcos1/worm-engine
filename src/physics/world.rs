    ...
    pub fn step(&mut self) {
        ...
        self.bodies.shapes[..exact_len].par_chunks_mut(chunk_size)
            ...
            .for_each(|((((((((((shapes, masses), v_x), v_y), v_z), a_x), a_y), a_z), f_x), f_y), f_z)| {
                ...
                // Load forces
                let mut force_x = f32x4::from([f_x[0], f_x[1], f_x[2], f_x[3]]);
                let mut force_y = f32x4::from([f_y[0], f_y[1], f_y[2], f_y[3]]);
                let mut force_z = f32x4::from([f_z[0], f_z[1], f_z[2], f_z[3]]);

                // Apply gravity (F = F + mg)
                force_x = force_x + (grav_x * mass_simd);
                force_y = force_y + (grav_y * mass_simd);
                force_z = force_z + (grav_z * mass_simd);

                // Calculate acceleration (a = F / m)
                let accel_x = force_x * inv_mass;
                let accel_y = force_y * inv_mass;
                let accel_z = force_z * inv_mass;

                // Load velocities
                let mut vel_x = f32x4::from([v_x[0], v_x[1], v_x[2], v_x[3]]);
                let mut vel_y = f32x4::from([v_y[0], v_y[1], v_y[2], v_y[3]]);
                let mut vel_z = f32x4::from([v_z[0], v_z[1], v_z[2], v_z[3]]);

                // Update velocities (v = v + a * dt)
                vel_x = vel_x + (accel_x * dt_simd);
                vel_y = vel_y + (accel_y * dt_simd);
                vel_z = vel_z + (accel_z * dt_simd);

                // Store back
                let a_x_arr: [f32; 4] = accel_x.into();
                let a_y_arr: [f32; 4] = accel_y.into();
                let a_z_arr: [f32; 4] = accel_z.into();

                let v_x_arr: [f32; 4] = vel_x.into();
                let v_y_arr: [f32; 4] = vel_y.into();
                let v_z_arr: [f32; 4] = vel_z.into();
                ...
                for i in 0..4 {
                    ...
                    let velocity = Vector3d::new(v_x[i], v_y[i], v_z[i]);
                    // Update vertices
        ...
        if remainder > 0 {
            ...
            for i in start..end {
                ...
                let force_x = self.bodies.forces_x[i] + crate::physics::constants::GRAVITY.x * mass;
                let force_y = self.bodies.forces_y[i] + crate::physics::constants::GRAVITY.y * mass;
                let force_z = self.bodies.forces_z[i] + crate::physics::constants::GRAVITY.z * mass;
                ...
                self.bodies.velocities_x[i] += accel_x * dt;
                self.bodies.velocities_y[i] += accel_y * dt;
                self.bodies.velocities_z[i] += accel_z * dt;

                let velocity = Vector3d::new(self.bodies.velocities_x[i], self.bodies.velocities_y[i], self.bodies.velocities_z[i]);
