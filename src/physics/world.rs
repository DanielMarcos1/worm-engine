    ...
    pub fn step(&mut self) {
        ...
        for i in 0..num_bodies {
            for j in (i + 1)..num_bodies {
                ...
                if self.shapes[i].vertices.is_empty() || self.shapes[j].vertices.is_empty() {
                    continue;
                }
                ...
                if let Some(toi) = calculate_toi_sphere_sphere(p1, v1, r1, p2, v2, r2, dt) {
                    ...
                    if v1_proj < 0.0 {
                        self.velocities[i] =
                            self.velocities[i].subtract(&collision_normal.scale(v1_proj));
                    }
                    if v2_proj > 0.0 {
                        self.velocities[j] =
                            self.velocities[j].subtract(&collision_normal.scale(v2_proj));
                    }
                }
            }
        }
